...
export const deleteTweet = async (req, res) => {
  ...
  if (!refreshToken || !id) return res.status(404).send();
  const _id = new ObjectId(id);
  const user = await users.findOne({ refreshToken: refreshToken });
  if (!user) return res.status(409).send();
  const tweetToDelete = await tweets.findOne({ _id: _id });
  if (!tweetToDelete) return res.status(404).send();
  await tweets.deleteOne({ _id: _id });
  return res.status(200).send();
};
